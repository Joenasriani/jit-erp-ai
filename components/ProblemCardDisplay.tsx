...
const DetailCard: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
        <h4 className="text-md font-semibold text-indigo-400 border-b border-gray-600 pb-2 mb-3">{title}</h4>
        {children}
    </div>
);
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-3">
            <DetailCard title="Entities">
                ...
                {card.entities.map((entity: Entity) => (
                    <div key={entity.name} className="flex-1 min-w-[250px] bg-gray-900 p-3 rounded-md border border-gray-700">
                        <p className="font-bold text-cyan-400">{entity.name}</p>
                        <ul className="list-disc list-inside text-sm text-gray-400 pl-2 mt-1">
                            {entity.fields.map(field => <li key={field.name}>{field.name}: <span className="text-gray-300">{field.type}</span></li>)}
                        </ul>
                    </div>
                ))}
                </div>
             ...
             <ul className="space-y-2 text-gray-300">
                {card.relationships.map((rel: Relationship, i) => (
                    <li key={i} className="flex items-center gap-2">
                        <span className="font-mono bg-gray-700 px-2 py-1 rounded-md text-sm">{rel.from}</span>
                        <span className="text-indigo-400">&rarr;</span>
                        <span className="font-mono bg-gray-700 px-2 py-1 rounded-md text-sm">{rel.to}</span>
        ...
        <DetailCard title="Actions">
             <ul className="space-y-2 text-gray-300">
                {card.actions.map((action: Action) => <li key={action.name} className="font-mono text-sm bg-gray-700/50 px-2 py-1 rounded">{action.name}</li>)}
            </ul>
        </DetailCard>

        {/* Policies & Compliance */}
        <div className="space-y-6">
            <DetailCard title="Policies">
                <ul className="space-y-2 text-gray-300">
                    {card.policies.map((policy: Policy) => (
                        <li key={policy.role}><span className="font-semibold text-cyan-400">{policy.role}:</span> Can perform {policy.actions.length} actions.</li>
                    ))}
                </ul>
            </DetailCard>
            <DetailCard title="Compliance">
                <div className="flex flex-wrap gap-2">
                {card.compliance.length > 0 ? card.compliance.map(c => (
                    <span key={c} className="bg-green-800/50 text-green-300 text-xs font-medium px-2.5 py-0.5 rounded-full border border-green-700">{c}</span>
